...
<br>
<div class="container">
	<h2 style="text-align: center;">Edit {{$customer->customer_name}}</h2>
	<hr>
	<div class="row">
		...
		<div class="col-sm-8">
			<form action="{{route('customer.update', $customer->id)}}" method="POST">
				...
				<div class="form-group{{ $errors->has('customer_mobile') ? ' has-error' : '' }}">
					...
					<input type="text" class="form-control" id="customer_mobile" name="customer_mobile" value="{{ $customer->customer_mobile }}" placeholder="Customer Mobile">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" id="customer_email" name="customer_email" value="{{ $customer->customer_email }}" placeholder="Customer Email">
				</div>
				...
				<div class="form-group{{ $errors->has('area_id') ? ' has-error' : '' }}">
					...
					<select id="area_id" name="area_id" class="form-control">
						...
						<option value="{{ $area->id }}" @if($area->id==$customer->area_id) selected='selected' @endif>{{ $area->area_name }}</option>
						@endforeach
					</select>
				...
				<div class="form-group{{ $errors->has('trade_price') ? ' has-error' : '' }}">
					...
					<input type="text" class="form-control" id="trade_price" name="trade_price" value="{{ $customer->trade_price }}" placeholder="Trade Price">
				...
				<div class="form-group">
					...
					<textarea class="form-control" id="customer_address" name="customer_address" placeholder="Customer Address" rows="5" cols="50">{{ $customer->customer_address }}</textarea>
				</div>
				<button type="submit" class="btn btn-success">Update</button>
				<button type="button" class="btn btn-danger" onClick="window.location.href='/customer'">Cancel</button>
			</form>
		</div>
